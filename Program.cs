...

{
...

    {
        ...
        public void Serialize(FileStream s)
        {
            ...
            using (StreamWriter w = new StreamWriter(s))
                ...
                foreach (ListNode n in arr)
                {
                    ...
                    w.WriteLine(n.Data.ToString() + ":" + n.GetHashCode().ToString() + ":" + n.Rand.GetHashCode().ToString()); 
                }
        }
        ...
        {
            //List<ListNode> arr = new List<ListNode>();
            Dictionary<int, ListNode> arr = new Dictionary<int, ListNode>();
            ...
            try
            {
                using (StreamReader sr = new StreamReader(s))
                {
                    while ((line = sr.ReadLine()) != null)
                    {
                        if (!line.Equals(""))
                        {
                            ...
                            temp.Next = next;
                            string[] currentData = temp.Data.Split(':');
                            int key = Convert.ToInt32(currentData[1]);
                            arr.Add(key, temp);
                            next.Prev = temp;
                            temp = next;
                        }
                ...
                foreach (ListNode n in arr.Values)
                {
                    n.Rand = arr[Convert.ToInt32(n.Data.Split(':')[2])];
